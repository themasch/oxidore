...
macro_rules! protocol {
    (
        ...
        impl PacketFactory {
            pub fn unpack(state: &ConnectionState, info: PacketInformation, data: &[u8]) -> Result<McPacket, ()>
            {
                match state {
                    $(
                        &ConnectionState::$valid_state => {
                            match info.id {
                                $(
                                    $package_id => $name::unpack(data),
                                )+
                                _ => Err(())
                            }
                        },
